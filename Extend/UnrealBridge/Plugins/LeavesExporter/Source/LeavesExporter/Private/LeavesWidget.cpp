...
static void OnComponentComplete(PaintsNow::IScript::Request& request) {

}

static void OnEntityComplete(PaintsNow::IScript::Request& request) {
...
static void PostResource(PaintsNow::Service& service, PaintsNow::ResourceBase& resource, const PaintsNow::String& location) {
	...
	*f << resource;
	...
	request.Push();
	service->PostResource(request.Adapt(Wrap(OnResourceComplete)), request, location, extension, data);
	request.Pop();
}
...
static void CompleteSync(PaintsNow::Service& service) {
	...
	auto& request = service.GetMainRequest();
	request.Push();
	service->Complete(request.Adapt(Wrap(OnResourceComplete)), request);
	request.Pop();
}
...
void SLeavesWidget::CollectSceneEntity(AActor& actor) {
	...
	request.Push();
	(*service)->PostEntity(request.Adapt(Wrap(OnEntityComplete)), request, entityID, 0, actorName);
	request.Pop();
	request.Push();
	(*service)->PostTransformComponent(
		request.Adapt(Wrap(OnComponentComplete)),
		request,
		componentCount,
		ConvertVector(transform.GetLocation() * FVector(-1, 1, 1) / 100.0f),
		ConvertVector(transform.GetScale3D()),
		ConvertVector(transform.GetRotation().Euler() / 180.0f * PI));
	request.Pop();
	request.Push();
	(*service)->PostEntityComponent(request.Adapt(Wrap(OnComponentComplete)), request, entityID, componentCount);
	request.Pop();
...
void SLeavesWidget::OnExportMeshComponent(UMeshComponent* meshComponent) {
	...
	if (meshComponent->IsA(UStaticMeshComponent::StaticClass())) {
		...
		if (staticMesh != nullptr) {
				...
				for (int32 i = 0; i < lodResources.Num(); i++) {
					...
					for (int32 k = 0; k < mesh.Sections.Num(); k++) {
						...
						if (mat < materialResources.size()) {
							...
							request.Push();
							(*service)->PostModelComponentMaterial(request.Adapt(Wrap(OnComponentComplete)), request, modelComponentID, k, materialResource->GetLocation());
							request.Pop();
						}
					}
					...
					request.Push();
					(*service)->PostModelComponent(request.Adapt(Wrap(OnComponentComplete)), request, modelComponentID, meshResource->GetLocation(), 0);
					request.Pop();
			...
			IScript::Request& request = service->GetMainRequest();
			request.Push();
			(*service)->PostEntityComponent(request.Adapt(Wrap(OnEntityComplete)), request, entityCount, mapModelComponent[staticMesh]);
			request.Pop();
		}
	}
}
...
void SLeavesWidget::OnExportSphereReflectionComponent(UReflectionCaptureComponent* sphereReflectionComponent) {
	...
	request.Push();
	(*service)->PostEnvCubeComponent(request.Adapt(Wrap(OnComponentComplete)), request, componentCount, textureName);
	request.Pop();

	request.Push();
	(*service)->PostEntityComponent(request.Adapt(Wrap(OnEntityComplete)), request, entityCount, componentCount++);
	request.Pop();
}
