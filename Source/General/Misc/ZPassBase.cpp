...
TObject<IReflect>& ZPassBase::operator () (IReflect& reflect) {
...
IRender::Resource* ZPassBase::Compile(IRender& render, IRender::Queue* queue) {
...
ZPassBase::Updater::Updater(ZPassBase& Pass) : textureCount(0), IReflect(true, false, false, false) {
	Pass(*this);
	Flush();
}
...
ZPassBase::Parameter& ZPassBase::Updater::operator [] (IShader::BindInput::SCHEMA schema) {
	static Parameter defOutput;
	Name input;
	input.schema = schema;
	std::map<Name, size_t>::iterator it = mapParameters.find(input);
...
void ZPassBase::Updater::Property(IReflectObject& s, Unique typeID, Unique refTypeID, const char* name, void* base, void* ptr, const MetaChainBase* meta) {
	Name input;
	Bytes byteName;
...
void ZPassBase::Updater::Update(IRender& render, IRender::Queue* queue, IRender::Resource::DrawCallDescription& drawCallDescription, std::vector<IRender::Resource*>& newBuffers, std::vector<Bytes>& bufferData, uint32_t bufferMask) {
...
Bytes ZPassBase::ExportHash(bool constantOnly) {
	if (constantOnly) {
...
bool ZPassBase::Name::operator < (const Name& rhs) const {
...
Bytes ZPassBase::PartialUpdater::ComputeHash() const {
	...
	for (size_t i = 0; i < parameters.size(); i++) {
		const ZPassBase::Parameter& output = parameters[i];
...
public:
	ReflectPartial(ZPassBase::Updater& u) : IReflect(true), updater(u) {}

	virtual void Property(IReflectObject& s, Unique typeID, Unique refTypeID, const char* name, void* base, void* ptr, const MetaChainBase* meta) {
		if (s.IsBasicObject()) {
			ZPassBase::Parameter* m = nullptr;
			...
			Bytes byteName;
			byteName.Assign((const uint8_t*)name, safe_cast<uint32_t>(strlen(name)));
			ZPassBase::Parameter& parameter = m != nullptr && *m ? *m : updater[byteName];
