...
void vpush64(int ty, uint64_t v)
{
    CValue cval;
    CType ctype;
    ctype.t = ty;
...
static inline void vpushll(int64_t v)
{
    vpush64(VT_LLONG, v);
}
...
ST_FUNC int get_reg(int rc)
{
    ...
    for(p=vstack;p<=vtop;p++) {
	/* look at second register (if int64_t) */
	r = p->r2 & VT_VALMASK;
	    ...
	    )
	{
		...
		if ((vtop->r & (VT_VALMASK | VT_LVAL)) == VT_CONST) {
		    ...
		    vpushi(ll >> 32); /* second word */
			   ...
			   (vtop->r & VT_LVAL)) {
		    /* We do not want to modifier the int64_t
...
/* expand int64_t on stack in two int registers */
...
#ifndef TCC_TARGET_X86_64
/* generate CPU independent (unsigned) int64_t operations */
...
ST_FUNC void gen_op(int op)
{
    ...
    if (bt1 == VT_PTR || bt2 == VT_PTR) {
        ...
        /* cast to biggest op */
        t = VT_LLONG;
        /* convert to unsigned if it does not fit in a int64_t */
    ...
    std_op:
        ...
        gen_cast(&type1);
        vswap();
        /* special case for shifts and int64_t: we keep the shift as
           an integer */
...
/* generic itof for uint64_t case */
...
/* generic ftoi for uint64_t case */
    ...
    if (sbt != dbt) {
        ...
        if (c) {
            ...
            if (df) {
                ...
                vtop->c.ull = (uint64_t)vtop->c.ld;
            } else if (sf && dbt == VT_BOOL) {
                vtop->c.i = (vtop->c.ld != 0);
            } else {
                if(sf)
                    vtop->c.ll = (int64_t)vtop->c.ld;
            ...
            if (sf && df) {
                ...
                /* scalar to int */
                if (sbt == VT_LLONG) {
                    /* from int64_t: just take low order word */
                    lexpand();
                    vpop();
                } 
    ...
    if (sbt == VT_STRUCT) {
        ...
        if((ft & VT_BTYPE) != VT_BOOL) {
            if((ft & VT_BTYPE) == VT_LLONG) {
                vpushll(((uint64_t)1 << bit_size) - (uint64_t)1);
            } else {
                vpushi((1 << bit_size) - 1);
            }
            gen_op('&');
        ...
        vswap();
        if((ft & VT_BTYPE) == VT_LLONG) {
            vpushll(~((((uint64_t)1 << bit_size) - (uint64_t)1) << bit_pos));
        } else {
...
ST_FUNC void unary(void)
{
 ...
 tok_next:
    ...
    case TOK___NAN__:
        ...
        break;
    case TOK___SNAN__:
        vpush64(VT_DOUBLE, 0x7ff8000000000001);
        next();
        break;
    case TOK___INF__:
		...
		if (is_null_pointer (vtop))
		  ...
		  type = type1;
            ...
            if (is_float(type.t)) {
                ...
                /* for int64_ts, we use fixed registers to avoid having
