// Starting with OpenGL as template ...
...
bool VulkanErrorGuard::enableGuard = true;

#define GL_GUARD() VulkanErrorGuard guard;

class DeviceImplVulkan : public IRender::Device {
public:
	DeviceImplVulkan(IRender& r) : lastProgramID(0), lastFrameBufferID(0), render(r) {}
...
struct QueueImplVulkan;
struct ResourceBaseImplVulkan : public ResourceAligned {
	virtual ~ResourceBaseImplVulkan() {}
	...
	virtual void Nop(QueueImplVulkan& queue) {}
	virtual void Execute(QueueImplVulkan& queue) = 0;
	virtual void Upload(QueueImplVulkan& queue) = 0;
	virtual void Download(QueueImplVulkan& queue) = 0;
	virtual void PreSwap(QueueImplVulkan& queue) = 0;
	virtual void PostSwap(QueueImplVulkan& queue) = 0;
	virtual void SyncDownload(QueueImplVulkan& queue) = 0;
	virtual void Delete(QueueImplVulkan& queue) = 0;
};

struct ResourceCommandImplVulkan {
	...
	ResourceCommandImplVulkan(Operation a = OP_EXECUTE, IRender::Resource* r = nullptr) {
	...
	bool Invoke(QueueImplVulkan& queue) {
		// decode mask	
		static Action actionTable[] = {
			&ResourceBaseImplVulkan::Nop,
			&ResourceBaseImplVulkan::Execute,
			&ResourceBaseImplVulkan::Upload,
			&ResourceBaseImplVulkan::Download,
			&ResourceBaseImplVulkan::Delete,
			&ResourceBaseImplVulkan::PreSwap,
			&ResourceBaseImplVulkan::PostSwap
		};
		...
		ResourceBaseImplVulkan* impl = static_cast<ResourceBaseImplVulkan*>(GetResource());
...
struct QueueImplVulkan : public IRender::Queue {
	QueueImplVulkan(DeviceImplVulkan* d, bool shared) : device(d) {
		critical.store(shared ? 0u : ~(uint32_t)0u, std::memory_order_relaxed);
	}

	inline void QueueCommand(const ResourceCommandImplVulkan& command) {
	...
	inline void Merge(QueueImplVulkan* source) {
		while (!source->queuedCommands.Empty()) {
			ResourceCommandImplVulkan command = source->queuedCommands.Top();
			source->queuedCommands.Pop();
			queuedCommands.Push(command);
		}
	}

	static inline void PrefetchCommand(const ResourceCommandImplVulkan& predict) {
	...
	struct Scanner {
		Scanner(QueueImplVulkan& q) : queue(q) {
#ifdef _DEBUG
			yielded = false;
			count = 0;
#endif
		...
		bool operator () (ResourceCommandImplVulkan& command, ResourceCommandImplVulkan& predict) {
			PrefetchCommand(predict);
			if (command.Invoke(queue)) {
#ifdef _DEBUG
				count++;
#endif
				if (command.GetOperation() != ResourceCommandImplVulkan::OP_EXECUTE) {
					// replace it with nop
					command = ResourceCommandImplVulkan(ResourceCommandImplVulkan::OP_NOP, nullptr);
				}
	...
	void ExecuteAll() {
		ResourceCommandImplVulkan command;
		while (!queuedCommands.Empty()) {
			ResourceCommandImplVulkan& command = queuedCommands.Top();
	...
	void ClearAll() {
		while (!queuedCommands.Empty()) {
			ResourceCommandImplVulkan command = queuedCommands.Top();
			queuedCommands.Pop();
			if (command.GetOperation() == ResourceCommandImplVulkan::OP_DELETE) {
				command.Invoke(*this);
			}
		}
	}
	...
	void ConsumeYield() {
		while (!queuedCommands.Empty()) {
			ResourceCommandImplVulkan command = queuedCommands.Top();
			queuedCommands.Pop();
			if (command.GetOperation() != ResourceCommandImplVulkan::OP_NOP) {
				if (command.GetResource() == nullptr) { // yield?
					return;
				} else if (command.GetOperation() != ResourceCommandImplVulkan::OP_EXECUTE) {
					command.Invoke(*this);
				}
			}
		}
	...
	ResourceBaseImplVulkan* swapResource;
	TQueueList<ResourceCommandImplVulkan> queuedCommands;
};
...
template <class T>
struct ResourceBaseImplVulkanDesc : public ResourceBaseImplVulkan {
	typedef ResourceBaseImplVulkanDesc<T> Base;
	ResourceBaseImplVulkanDesc() : downloadDescription(nullptr) {
	...
	virtual void Download(QueueImplVulkan& queue) override {
		assert(downloadDescription != nullptr);
		*downloadDescription = currentDescription;
	}
	
	virtual void SyncDownload(QueueImplVulkan& queue) override {
		downloadDescription = nullptr;
	}

	virtual void PreSwap(QueueImplVulkan& queue) override { assert(false); } // not implemented by default
	virtual void PostSwap(QueueImplVulkan& queue) override { assert(false); }
...
template <class T>
struct ResourceImplVulkan : public ResourceBaseImplVulkanDesc<T> {
	virtual IRender::Resource::Type GetType() const  override { return IRender::Resource::RESOURCE_UNKNOWN; }
	virtual void Upload(QueueImplVulkan& queue) override { assert(false); }
	virtual void Download(QueueImplVulkan& queue) override { assert(false); }
	virtual void Execute(QueueImplVulkan& queue) override { assert(false); }
	virtual void Delete(QueueImplVulkan& queue) override { assert(false); }
};

template <>
struct ResourceImplVulkan<IRender::Resource::TextureDescription> : public ResourceBaseImplVulkanDesc<IRender::Resource::TextureDescription> {
	ResourceImplVulkan() : textureID(0), textureType(GL_TEXTURE_2D), pixelBufferObjectID(0) {}
	...
	virtual void Upload(QueueImplVulkan& queue) override {
		GL_GUARD();
	...
	virtual void Execute(QueueImplVulkan& queue) override {} // Not an executable resource
	virtual void Delete(QueueImplVulkan& queue) override {
		GL_GUARD();
	...
	virtual void PreSwap(QueueImplVulkan& queue) override {
		queue.swapResource = this;
	}

	virtual void PostSwap(QueueImplVulkan& queue) override {
		ResourceImplVulkan<TextureDescription>* t = static_cast<ResourceImplVulkan<TextureDescription>*>(queue.swapResource);
...
template <>
struct ResourceImplVulkan<IRender::Resource::BufferDescription> : public ResourceBaseImplVulkanDesc<IRender::Resource::BufferDescription> {
	ResourceImplVulkan() : bufferID(0) {}
...
class ShaderDeclarationVulkan : public IReflect {
public:
	ShaderDeclarationVulkan(IRender::Resource::ShaderDescription::Stage s, uint32_t& pinputIndex, uint32_t& poutputIndex, uint32_t& ptextureIndex) : IReflect(true, false), stage(s), inputIndex(pinputIndex), outputIndex(poutputIndex), textureIndex(ptextureIndex) {}
	void Complete() {
...
template <>
struct ResourceImplVulkan<IRender::Resource::ShaderDescription> : public ResourceBaseImplVulkanDesc<IRender::Resource::ShaderDescription> {
	ResourceImplVulkan() {}
	...
	virtual void Upload(QueueImplVulkan& queue) override {
		...
		for (size_t k = 0; k < Resource::ShaderDescription::END; k++) {
			...
			for (size_t n = 0; n < pieces.size(); n++) {
				...
				ShaderDeclarationVulkan declaration((Resource::ShaderDescription::Stage)k, inputIndex, outputIndex, textureIndex);
				(*shader)(declaration);
			...
			if (success == 0) {
				...
				fprintf(stderr, "ZRenderVulkan::CompileShader(): %s\n", info);
				fprintf(stderr, "ZRenderVulkan::CompileShader(): %s\n", fullShader.c_str());
				// assert(false);
		...
		if (success == 0) {
			...
			fprintf(stderr, "ZRenderVulkan::LinkProgram(): %s\n", info);
			if (pass.compileCallback) {
...
template <>
struct ResourceImplVulkan<IRender::Resource::RenderStateDescription> : public ResourceBaseImplVulkanDesc<IRender::Resource::RenderStateDescription> {
	...
	virtual void Execute(QueueImplVulkan& queue) override {
		GL_GUARD();
...
template <>
struct ResourceImplVulkan<IRender::Resource::ClearDescription> : public ResourceBaseImplVulkanDesc<IRender::Resource::ClearDescription> {
	...
	virtual void Download(QueueImplVulkan& queue) override {}
	virtual void Delete(QueueImplVulkan& queue) override { delete this; }
...
template <>
struct ResourceImplVulkan<IRender::Resource::RenderTargetDescription> : public ResourceBaseImplVulkanDesc<IRender::Resource::RenderTargetDescription> {
	ResourceImplVulkan() : vertexArrayID(0), frameBufferID(0) {}
	...
	virtual void SetUploadDescription(IRender::Resource::Description* d) override {
		...
		for (size_t i = 0; i < t->colorBufferStorages.size(); i++) {
			ResourceImplVulkan<IRender::Resource::TextureDescription>* x = static_cast<ResourceImplVulkan<IRender::Resource::TextureDescription>*>(t->colorBufferStorages[i].resource);
			assert(x->GetNextDescription().dimension.x() != 0 && x->GetNextDescription().dimension.y() != 0);
		}
		ResourceBaseImplVulkanDesc<IRender::Resource::RenderTargetDescription>::SetUploadDescription(d);
	}
#endif //
	...
	virtual void Upload(QueueImplVulkan& queue) override {
		...
		if (!d.colorBufferStorages.empty()) {
			...
			if (d.depthStencilStorage.resource == nullptr) {
				...
				ResourceImplVulkan<IRender::Resource::TextureDescription>* t = static_cast<ResourceImplVulkan<IRender::Resource::TextureDescription>*>(d.depthStencilStorage.resource);
			...
			for (size_t i = 0; i < d.colorBufferStorages.size(); i++) {
				...
				ResourceImplVulkan<IRender::Resource::TextureDescription>* t = static_cast<ResourceImplVulkan<IRender::Resource::TextureDescription>*>(storage.resource);
				assert(t != nullptr);
				assert(t->textureID != 0);
				if (t->GetDescription().state.sample == IRender::Resource::TextureDescription::RENDERBUFFER) {
...
#ifdef LOG_OPENGL
				printf("[Vulkan ColorAttach] %d = %p from %p\n", i, t, this);
#endif
			}
	...
	virtual void Execute(QueueImplVulkan& queue) override {
		...
		if (range.second.x() == 0) {
			if (d.colorBufferStorages.empty()) {
				...
				ResourceImplVulkan<TextureDescription>* texture = static_cast<ResourceImplVulkan<TextureDescription>*>(d.colorBufferStorages[0].resource);
...
template <>
struct ResourceImplVulkan<IRender::Resource::DrawCallDescription> : public ResourceBaseImplVulkanDesc<IRender::Resource::DrawCallDescription> {
	...
	virtual void Execute(QueueImplVulkan& queue) override {
		...
		typedef ResourceImplVulkan<ShaderDescription> Shader;
		typedef ResourceImplVulkan<BufferDescription> Buffer;
		typedef DrawCallDescription::BufferRange BufferRange;
		typedef ResourceImplVulkan<TextureDescription> Texture;
...
template <>
struct ResourceImplVulkan<IRender::Resource::NotifyDescription> : public ResourceBaseImplVulkanDesc<IRender::Resource::NotifyDescription> {
	virtual IRender::Resource::Type GetType() const override { return RESOURCE_DRAWCALL; }
	virtual void Upload(QueueImplVulkan& queue) override {
		UpdateDescription();
	}
...
IRender::Device* ZRenderVulkan::CreateDevice(const String& description) {
	if (description.empty()) {
		return new DeviceImplVulkan(*this); // by now we only supports one device
	} else {
		return nullptr;
	}
}
...
// Queue
IRender::Queue* ZRenderVulkan::CreateQueue(Device* device, bool shared) {
	assert(device != nullptr);
	return new QueueImplVulkan(static_cast<DeviceImplVulkan*>(device), shared);
}

IRender::Device* ZRenderVulkan::GetQueueDevice(Queue* queue) {
...
void ZRenderVulkan::PresentQueues(Queue** queues, uint32_t count, PresentOption option) {
	GL_GUARD();
	void (QueueImplVulkan::*op)() = &QueueImplVulkan::ExecuteAll;
	switch (option) {
	case PresentOption::PRESENT_EXECUTE_ALL:
		op = &QueueImplVulkan::ExecuteAll;
		break;
	case PresentOption::PRESENT_REPEAT_TO_YIELD:
		op = &QueueImplVulkan::RepeatToYield;
		break;
	case PresentOption::PRESENT_EXECUTE_TO_YIELD:
		op = &QueueImplVulkan::ExecuteToYield;
		break;
	case PresentOption::PRESENT_CONSUME_YIELD:
		op = &QueueImplVulkan::ConsumeYield;
		break;
	case PresentOption::PRESENT_CLEAR_ALL:
		op = &QueueImplVulkan::ClearAll;
		break;
	}

	for (uint32_t k = 0; k < count; k++) {
		QueueImplVulkan* q = static_cast<QueueImplVulkan*>(queues[k]);
		(q->*op)();
	}
...
bool ZRenderVulkan::IsQueueEmpty(Queue* queue) {
	QueueImplVulkan* q = static_cast<QueueImplVulkan*>(queue);
...
void ZRenderVulkan::MergeQueue(Queue* target, Queue* source) {
	...
	QueueImplVulkan* t = static_cast<QueueImplVulkan*>(target);
	QueueImplVulkan* s = static_cast<QueueImplVulkan*>(source);
...
void ZRenderVulkan::YieldQueue(Queue* queue) {
	QueueImplVulkan* q = static_cast<QueueImplVulkan*>(queue);
	assert(queue != nullptr);
	q->QueueCommand(ResourceCommandImplVulkan(ResourceCommandImplVulkan::OP_EXECUTE, nullptr));
}
...
// Resource
IRender::Resource* ZRenderVulkan::CreateResource(Queue* queue, Resource::Type resourceType) {
	switch (resourceType) {
	...
	case Resource::RESOURCE_NOTIFY:
		return new ResourceImplVulkan<Resource::NotifyDescription>();
	}
...
void ZRenderVulkan::UploadResource(Queue* queue, Resource* resource, Resource::Description* description) {
	...
	q->QueueCommand(ResourceCommandImplVulkan(ResourceCommandImplVulkan::OP_UPLOAD, resource));
}
...
void ZRenderVulkan::RequestDownloadResource(Queue* queue, Resource* resource, Resource::Description* description) {
	...
	q->QueueCommand(ResourceCommandImplVulkan(ResourceCommandImplVulkan::OP_DOWNLOAD, resource));
}
...
void ZRenderVulkan::CompleteDownloadResource(Queue* queue, Resource* resource) {
	...
	ResourceBaseImplVulkan* impl = static_cast<ResourceBaseImplVulkan*>(resource);
	impl->SyncDownload(*q);
	// q->QueueCommand(ResourceCommandImplVulkan(ResourceCommandImplVulkan::OP_SYNC_DOWNLOAD, resource));
}
...
void ZRenderVulkan::ExecuteResource(Queue* queue, Resource* resource) {
	...
	assert(resource != nullptr);
	QueueImplVulkan* q = static_cast<QueueImplVulkan*>(queue);
	q->QueueCommand(ResourceCommandImplVulkan(ResourceCommandImplVulkan::OP_EXECUTE, resource));
}
...
void ZRenderVulkan::SwapResource(Queue* queue, Resource* lhs, Resource* rhs) {
	...
	assert(rhs != nullptr);
	QueueImplVulkan* q = static_cast<QueueImplVulkan*>(queue);
	q->QueueCommand(ResourceCommandImplVulkan(ResourceCommandImplVulkan::OP_PRESWAP, lhs));
	q->QueueCommand(ResourceCommandImplVulkan(ResourceCommandImplVulkan::OP_POSTSWAP, rhs));
}
...
void ZRenderVulkan::DeleteResource(Queue* queue, Resource* resource) {
	...
	assert(resource != nullptr);
	QueueImplVulkan* q = static_cast<QueueImplVulkan*>(queue);
	q->QueueCommand(ResourceCommandImplVulkan(ResourceCommandImplVulkan::OP_DELETE, resource));
}
...
void ZRenderVulkan::ClearDeletedQueues() {
...
ZRenderVulkan::~ZRenderVulkan() {
	VulkanErrorGuard::enableGuard = false;
	ClearDeletedQueues();
}
