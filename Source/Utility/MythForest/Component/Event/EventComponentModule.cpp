...
void EventComponentModule::RequestBindEventUserInput(IScript::Request& request, IScript::Delegate<EventComponent> eventComponent, bool enable) {
	...
	if (enable) {
		...
		SpinLock(critical);
		std::vector<TShared<EventComponent> >::iterator it = std::find(userInputs.begin(), userInputs.end(), eventComponent.Get());
		if (it == userInputs.end()) userInputs.emplace_back(eventComponent.Get());
		SpinUnLock(critical);
	} else {
		SpinLock(critical);
		std::vector<TShared<EventComponent> >::iterator it = std::find(userInputs.begin(), userInputs.end(), eventComponent.Get());
