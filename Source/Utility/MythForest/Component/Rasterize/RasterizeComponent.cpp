...
void RenderMeshEx(TypeTrait<D>, F, const TShared<TextureResource>& targetTexture, const TShared<MeshResource>& meshResource, const MatrixFloat4x4& transform) {
	...
	int32_t byteDepth = IImage::GetPixelSize(format, layout);
		...
		for (int32_t j = Math::Max(0, minY); j < Math::Min(maxY + 1, height); j++) {
			for (int32_t i = Math::Max(0, minX); i < Math::Min(maxX + 1, width); i++) {
				...
				if (getboolean<F>::value) {
					...
					source = (D)(Math::Clamp(depth, 0.0f, 1.0f) * (D)-1);
				}
...
void RasterizeComponent::RenderMesh(const TShared<MeshResource>& meshResource, const MatrixFloat4x4& transform) {
	...
	case IRender::Resource::TextureDescription::UNSIGNED_BYTE:
		RenderMeshEx(TypeTrait<uint8_t>(), std::false_type(), targetTexture, meshResource, transform);
		break;
	case IRender::Resource::TextureDescription::UNSIGNED_SHORT:
		RenderMeshEx(TypeTrait<uint16_t>(), std::false_type(), targetTexture, meshResource, transform);
		break;
	case IRender::Resource::TextureDescription::UNSIGNED_INT:
		RenderMeshEx(TypeTrait<uint32_t>(), std::false_type(), targetTexture, meshResource, transform);
		break;
	case IRender::Resource::TextureDescription::FLOAT:
		RenderMeshEx(TypeTrait<float>(), std::true_type(), targetTexture, meshResource, transform);
		break;
	default:
